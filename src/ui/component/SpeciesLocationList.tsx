...
const Tag: React.FC<Card_interface> = ({ speciesDeatail }) => {
    const randomColor = uniqolor.random().color // When deployment, we should use random color based on ID of the species
...
// Main component
interface SpeciesLocationList_interface {
    closeSpeciesLocationList: () => void,
    speciesDeatail: () => void,
    backToSpeciesList: () => void
}

const SpeciesLocationList: React.FC<SpeciesLocationList_interface> = ({
    closeSpeciesLocationList, speciesDeatail, backToSpeciesList
}) => {
    // State
            ...
            <div className="flex-1 w-full h-0 gap-2.5 flex flex-col px-mainTwoSidePadding">
                <span className="h-fit w-full flex justify-between items-center">
                    ...
                    <span className="flex gap-2.5">
                        ...
                        <button className="mainShadow !p-2.5 !rounded-small">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5">
...
export default SpeciesLocationList
