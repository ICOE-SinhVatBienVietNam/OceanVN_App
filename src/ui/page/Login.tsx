    ...
    return (
        <div className="shadowForm h-fit w-full bg-white p-5 rounded-main flex flex-col gap-4">
            ...
            <span className="flex flex-col items-center-safe gap-3">
                <button
                    className="w-full h-[40px] bg-mainLightBlue text-white !rounded-small"
                    onClick={handleSignin}
                >
                    Đăng nhập
                </button>
                <IonRouterLink href={routeConfig.register.root} className="!text-csNormal !text-mainDarkBlue italic underline">Chưa có tài khoản?</IonRouterLink>
            </span>
        </div>
    )
}
...
const Login: React.FC = () => {
    return <AuthenLayout form={LoginForm} />
